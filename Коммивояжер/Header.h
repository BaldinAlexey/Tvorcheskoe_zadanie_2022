...
template<class T>
void Graph<T>::Delete_this_Vertex()
{
...
template<class T>
int Graph<T>::Getter_kolvo_border()
{
	int amount = 0; 
	if (!this->IsEmpty()) 
	{ 
...
template<class T>
int Graph<T>::Getter_length(const T& vertex1, const T& vertex2)
{
	if (!this->IsEmpty()) 
	{
...
template<class T>
int Graph<T>::Getter_kolvo_veshin()
{
...
void draw_Text_for_menu(string text, int x1, int y1)
{
...
void button_click(int btn, int stat, int x, int y) 
{
	int move = 60;
	int height = 730;
	if (stat == GLUT_DOWN) 
...
void watch_display()
{
	...
	glutSwapBuffers(); // Свапаются буфферы обмена, так как матрицы были 
}
