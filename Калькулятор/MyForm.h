...
private: System::Void label6_Click(System::Object^ sender, System::EventArgs^ e) {
	   ...
	   bool flag = true; // булевая переменная, создана для вывода ошибки
	   bool Protect() // булевая функция, которая делает проверку на ввод, воспринимает цифры и знак минус
	   {
...
private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) // метод нажатия кнопки
{
	if (Protect()) // если проверка прошла
	{
		double Num1, Num2, Num3, Num4, C, C1, Result, chisl, znam; // создаем переменные: целая часть, числитель, знаменатель, результат
		Num1 = System::Convert::ToDouble(textBox1->Text); // Конвертируем полученный текст в double
		Num2 = System::Convert::ToDouble(textBox2->Text); // Конвертируем полученный текст в double
		Num3 = System::Convert::ToDouble(textBox3->Text); // Конвертируем полученный текст в double
		Num4 = System::Convert::ToDouble(textBox4->Text); // Конвертируем полученный текст в double
		C = System::Convert::ToDouble(textBox6->Text);    // Конвертируем полученный текст в double
		C1 = System::Convert::ToDouble(textBox7->Text);   // Конвертируем полученный текст в double

		/*/     Арифметические действия над дробями		/*/

		/*/		Работа с Дробями происходит так, как должна быть: Находим общий знаменатель. Если есть целая часть, умножаем целую часть на знаменатель + числитель	/*/
		...
		textBox5->Text = System::Convert::ToString(Result); // Приводим обратно в string переменную результат
	}
	else if (flag == false) { flag = true; return; } // иначе, ошибки нет, продолжаем вычислять
	else { MessageBox::Show("Все поля должны быть заполнены!", "Ошибка заполнения"); } // если нажали на кнопку вычислить, а какое-либо поле пустое - выводится эта ошибка

}

	private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) // метод, отвечающий за кнопку сбросить - всем поля становятся пустые
	{
		textBox1->Text = L"";
		textBox2->Text = L"";
		textBox3->Text = L"";
...
private: System::Void тёмнаяToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) // метод, который отвечает за кнопку - "Справка"
{
	MessageBox::Show("Автор - Балдин Алексей Игоревич\n" "Студент Группы : ИВТ - 21 - 1Б\n" "Университет - ПНИПУ\n ", "Данные Автора");
}


private: System::Void оПрограммеToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) // метод, который отвечает за кнопку - "о программе"
{
	MessageBox::Show("Это калькулятор обыкновенных дробей, которые могут содержать целую часть. Программа принимает для дроби три основных параметра: Целая часть, числитель, знаменатель. Для удобства, как и принято в математике, перевожу результат в десятичную дробь.\n\n\t\t           Успехов в вычислениях!", "Информация о программе");
}


private: System::Void MyForm_FormClosing(System::Object^ sender, System::Windows::Forms::FormClosingEventArgs^ e) // метод, который отвечает за кнопку крестик - проверка на выход из программы
{
	...
	{
		e->Cancel = true; // если "да", выход из программы - то же самое что и - Application::Exit();
	}
	else { e->Cancel = false; } // если "нет", продолжаем работу
}
